...
import { useCreateRegistration, useGetDashboardSummary, useListAnnouncements, useListScheduleItems, getGetDashboardSummaryQueryKey } from '@workspace/api-client-react';
import type { Announcement, RegistrationInput, ScheduleItem } from '@workspace/api-client-react';
import { useEffect, useMemo, useState } from 'react';
import { Link, Route, Router as WouterRouter, Switch, useLocation } from 'wouter';
import { ArrowDownRight, ArrowRight, Bell, CalendarDays, Check, ChevronDown, Clock3, Code2, ExternalLink, Github, Instagram, Mail, Menu, MapPin, Menu as MenuIcon, Network, Search, Sparkles, Trophy, Users, X, Zap } from 'lucide-react';
    ...
    <header className="relative z-40 border-b border-foreground/10 bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <span className="mono hidden text-[10px] uppercase tracking-[.18em] text-muted-foreground lg:inline">Oct 18, 2025 / Harper College</span>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-10 md:flex-row">
          ...
          <div className="flex items-start gap-3">
            <a href="mailto:hello@code211.dev" data-testid="link-footer-email" className="rounded-full border border-background/20 p-2.5 transition-colors hover:border-primary hover:text-primary"><Mail className="size-4" /></a>
            <a href="https://github.com" target="_blank" rel="noreferrer" data-testid="link-footer-github" className="rounded-full border border-background/20 p-2.5 transition-colors hover:border-primary hover:text-primary"><Github className="size-4" /></a>
            <a href="https://instagram.com" target="_blank" rel="noreferrer" data-testid="link-footer-instagram" className="rounded-full border border-background/20 p-2.5 transition-colors hover:border-primary hover:text-primary"><Instagram className="size-4" /></a>
          </div>
        </div>
        <div className="mono mt-14 flex flex-col justify-between gap-2 border-t border-background/15 pt-5 text-[10px] uppercase tracking-[.16em] text-background/40 sm:flex-row"><span>District 211 / student-run / 2025</span><span>Built in the northwest suburbs</span></div>
      </div>
    </footer>
  );
}
...
function Home() {
  const { data: next, isLoading: scheduleLoading } = useListScheduleItems();
  const { data: summary } = useGetDashboardSummary();
  const nextItem = summary?.nextEvent || next?.[0];
  return <Shell>
    <main>
      <section className="relative overflow-hidden bg-foreground px-5 pb-16 pt-14 text-background lg:px-8 lg:pb-24 lg:pt-20">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid gap-12 lg:grid-cols-[1.3fr_.7fr] lg:items-end">
            <div className="rise-in"><h1 className="display max-w-4xl text-[4.2rem] font-bold leading-[.85] tracking-[-.085em] sm:text-[7rem] lg:text-[9.2rem]">Make<br /><span className="text-primary">something</span><br /><span className="text-background/35">matter.</span></h1><p className="mt-9 max-w-md text-base leading-7 text-background/65 sm:text-lg">A one-day build sprint for high school students who have an idea, a laptop, and the nerve to ship it.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/register" data-testid="link-hero-register" className="group flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground transition-transform hover:-translate-y-1">Save your seat <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link><Link href="/about" data-testid="link-hero-about" className="rounded-full border border-background/25 px-5 py-3 text-sm font-semibold text-background transition-colors hover:border-primary hover:text-primary">How it works</Link></div></div>
            <div className="lg:pb-2"><div className="float rounded-[1.5rem] border border-background/15 bg-background/[.06] p-5 backdrop-blur"><div className="flex items-center justify-between border-b border-background/10 pb-4"><span className="mono text-[10px] uppercase tracking-[.18em] text-primary">Next up</span><span className="size-2 rounded-full bg-primary blink" /></div>{scheduleLoading ? <div className="mt-5 h-28 animate-pulse rounded-xl bg-background/10" /> : nextItem ? <><p className="mt-5 text-2xl font-bold leading-tight">{nextItem.title}</p><p className="mt-3 text-sm text-background/55">{nextItem.description}</p><div className="mono mt-6 flex items-center gap-2 text-xs text-primary"><Clock3 className="size-4" />{nextItem.startTime} — {nextItem.endTime}</div></> : <p className="mt-5 text-sm text-background/60">Schedule drops soon. Keep an eye on the live board.</p>}</div><div className="mono mt-5 flex items-center gap-2 text-[10px] uppercase tracking-[.16em] text-background/40"><MapPin className="size-3 text-secondary" /> Harper College / Building H</div></div>
          </div>
        </div>
      </section>
      <section className="border-b border-foreground/10 bg-primary px-5 py-5 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 sm:flex-row sm:items-center"><p className="font-bold tracking-[-.02em]">Registration is open for the next build day.</p><Link href="/register" data-testid="link-banner-register" className="flex items-center gap-2 text-sm font-bold underline decoration-foreground/30 underline-offset-4 hover:decoration-foreground">Get in the room <ArrowRight className="size-4" /></Link></div></section>
      <section className="px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><Eyebrow>Why code211</Eyebrow><h2 className="display max-w-md text-4xl font-bold leading-[.98] tracking-[-.06em] sm:text-5xl">Not a lecture.<br />A launchpad.</h2></div><div className="grid gap-x-8 gap-y-10 sm:grid-cols-2"><Feature icon={<Zap />} number="01" title="Start scrappy" text="No experience required. Come with a thought, or borrow one from the room." /><Feature icon={<Network />} number="02" title="Find your people" text="Build solo, bring a crew, or meet the teammate who makes your idea click." /><Feature icon={<Trophy />} number="03" title="Ship something real" text="Mentors, workshops, food, and a friendly deadline keep the momentum moving." /><Feature icon={<Sparkles />} number="04" title="Leave with proof" text="A working demo is the best first line on any future application." /></div></div></div></section>
...
function About() {
  return <Shell><PageIntro eyebrow="The brief / 01" title="A day to make your idea undeniable.">Code211 is a student-run hackathon for District 211 high school students. No gatekeeping, no pitch-deck theater — just a focused day of building, learning, and showing what you can do.</PageIntro><main>
...
function Schedule() {
  const query = useListScheduleItems();
  const [filter, setFilter] = useState<'all' | 'workshop'>('all');
  const items = useMemo(() => (query.data || []).filter((item) => filter === 'all' || item.kind === filter), [query.data, filter]);
  return <Shell><PageIntro eyebrow="The run of show / 02" title="One day. A lot can happen.">The schedule is a living thing. Check back for the latest workshops, mentor drops, breaks, and the moments that make the room feel electric.</PageIntro><main className="px-5 py-16 lg:px-8 lg:py-24"><div className="mx-auto max-w-5xl"><div className="mb-10 flex flex-wrap items-center justify-between gap-4"><div className="flex rounded-full border border-foreground/10 p-1"><button type="button" onClick={() => setFilter('all')} data-testid="button-filter-all" className={`rounded-full px-4 py-2 text-sm font-bold ${filter === 'all' ? 'bg-foreground text-background' : 'text-muted-foreground'}`}>Everything</button><button type="button" onClick={() => setFilter('workshop')} data-testid="button-filter-workshop" className={`rounded-full px-4 py-2 text-sm font-bold ${filter === 'workshop' ? 'bg-foreground text-background' : 'text-muted-foreground'}`}>Workshops</button></div><span className="mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">Harper College / Building H</span></div>{query.isLoading ? <LoadingCards count={5} /> : query.isError ? <ErrorState retry={() => query.refetch()} /> : items.length === 0 ? <div className="rounded-2xl border border-dashed border-foreground/20 p-12 text-center"><CalendarDays className="mx-auto size-8 text-primary" /><p className="mt-4 font-bold">The schedule is still warming up.</p><p className="mt-1 text-sm text-muted-foreground">Check back soon for workshops and event times.</p></div> : <div className="relative">{items.map((item, i) => <ScheduleRow key={item.id} item={item} first={i === 0} />)}</div>}</div></main></Shell>;
}

function ScheduleRow({ item, first }: { item: ScheduleItem; first: boolean }) {
  const tone = item.kind === 'workshop' ? 'bg-secondary text-secondary-foreground' : item.kind === 'break' ? 'bg-accent text-accent-foreground' : item.kind === 'judging' ? 'bg-foreground text-background' : 'bg-primary text-primary-foreground';
  return <div className="grid grid-cols-[86px_1fr] gap-5 border-t border-foreground/12 py-5 sm:grid-cols-[120px_1fr] sm:gap-8"><div className="mono pt-1 text-xs text-muted-foreground"><span className={first ? 'text-primary' : ''}>{item.startTime}</span><span className="block mt-1 opacity-50">{item.endTime}</span></div><div className="group relative rounded-2xl border border-foreground/10 bg-card p-5 transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-sm)]"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.12em] ${tone}`}>{item.kind}</span><span className="mono flex items-center gap-1 text-[10px] text-muted-foreground"><MapPin className="size-3" />{item.location}</span></div><h3 className="mt-4 text-lg font-bold">{item.title}</h3><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{item.description}</p></div></div>;
}
...
const faqs = [
  ...
  ['What should I build?', 'Build something you care about. A tool for your school, an experiment, a tiny game, a helpful website — the best idea is the one you want to keep working on.'],
...
function Register() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!form.name || !form.email || !form.school || !form.grade) { setError('Fill in the required fields so we know who to expect.'); return; }
    create.mutate({ data: { ...form, teamSize: Number(form.teamSize), teamName: form.teamName || null, projectIdea: form.projectIdea || null, dietaryNeeds: form.dietaryNeeds || null } }, { onSuccess: () => { setSubmitted(true); queryClient.invalidateQueries({ queryKey: getGetDashboardSummaryQueryKey() }); }, onError: () => setError('We could not save your registration. Check your details and try again.') });
  };
  if (submitted) return <Shell><main className="grid min-h-[70dvh] place-items-center px-5 py-20"><div className="rise-in max-w-xl text-center"><div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-[5px_5px_0_hsl(var(--secondary))]"><Check className="size-8" /></div><Eyebrow>Signal received</Eyebrow><h1 className="display text-5xl font-bold leading-none tracking-[-.07em] sm:text-7xl">You are<br /><span className="text-primary">in the room.</span></h1><p className="mx-auto mt-6 max-w-md text-muted-foreground">Your registration is saved. Keep October 18 open and watch the live board for updates.</p><div className="mt-8 flex justify-center gap-3"><Link href="/dashboard" data-testid="link-success-dashboard" className="rounded-full bg-foreground px-5 py-3 text-sm font-bold text-background">See the live board</Link><Link href="/" data-testid="link-success-home" className="rounded-full border px-5 py-3 text-sm font-bold">Back home</Link></div></div></main></Shell>;
  return <Shell><PageIntro eyebrow="Claim your spot / 04" title="Put your name on the build.">Registration takes about two minutes. You do not need a project plan — just enough curiosity to show up.</PageIntro><main className="px-5 py-16 lg:px-8 lg:py-24"><form onSubmit={submit} className="mx-auto max-w-4xl"><div className="grid gap-10 lg:grid-cols-[1fr_.7fr]"><div className="space-y-8"><FormSection label="About you"><div className="grid gap-5 sm:grid-cols-2"><Field label="Name" required value={form.name} onChange={(v) => update('name', v)} placeholder="Your full name" id="name" /><Field label="Email" required type="email" value={form.email} onChange={(v) => update('email', v)} placeholder="you@school.edu" id="email" /><Field label="School" required value={form.school} onChange={(v) => update('school', v)} placeholder="e.g. Palatine High School" id="school" /><SelectField label="Grade" required value={form.grade} onChange={(v) => update('grade', v)} id="grade" options={['9th grade', '10th grade', '11th grade', '12th grade']} /></div></FormSection><FormSection label="Your crew"><div className="grid gap-5 sm:grid-cols-2"><Field label="Team name" value={form.teamName || ''} onChange={(v) => update('teamName', v)} placeholder="Optional — name it later" id="teamName" /><SelectField label="Team size" value={form.teamSize} onChange={(v) => update('teamSize', v)} id="teamSize" options={['1', '2', '3', '4']} /></div></FormSection><FormSection label="The spark"><SelectField label="Experience level" value={form.experience} onChange={(v) => update('experience', v)} id="experience" options={['Just getting started', 'I have tried a few things', 'I build regularly', 'I am here to go further']} /><TextAreaField label="What might you build?" value={form.projectIdea || ''} onChange={(v) => update('projectIdea', v)} placeholder="A rough idea is perfect. What keeps crossing your mind?" id="projectIdea" /></FormSection><FormSection label="Make yourself comfortable"><TextAreaField label="Dietary needs" value={form.dietaryNeeds || ''} onChange={(v) => update('dietaryNeeds', v)} placeholder="Let us know how to feed you well." id="dietaryNeeds" /></FormSection></div><aside className="lg:pl-8"><div className="sticky top-8 rounded-[1.5rem] bg-foreground p-6 text-background"><Eyebrow color="text-primary">Before you hit send</Eyebrow><ul className="space-y-5 text-sm leading-6 text-background/65"><li className="flex gap-3"><Check className="mt-1 size-4 shrink-0 text-primary" />Registration is free for District 211 students.</li><li className="flex gap-3"><Check className="mt-1 size-4 shrink-0 text-primary" />No experience or finished idea required.</li><li className="flex gap-3"><Check className="mt-1 size-4 shrink-0 text-primary" />You can update your team details at the event.</li></ul>{error && <p className="mt-6 rounded-xl bg-destructive/20 p-3 text-sm text-red-200" data-testid="status-registration-error">{error}</p>}<button type="submit" disabled={create.isPending} data-testid="button-submit-registration" className="group mt-8 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3.5 text-sm font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60">{create.isPending ? 'Saving your spot…' : 'Submit registration'}<ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></button></div></aside></div></form></main></Shell>;
}
...
function Dashboard() {
  ...
  const schedule = useListScheduleItems();
  ...
  return <Shell><PageIntro eyebrow="Public signal / 05" title="The room, in real time.">Registration totals, the latest word from organizers, and the next thing on the schedule — all in one place.</PageIntro><main className="px-5 py-16 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><div className="grid gap-4 sm:grid-cols-3"><Stat icon={<Users />} label="Builders registered" value={summary.isLoading ? '—' : String(summary.data?.registrationCount ?? 0)} test="stat-registrations" /><Stat icon={<Network />} label="Teams forming" value={summary.isLoading ? '—' : String(summary.data?.teamCount ?? 0)} test="stat-teams" /><Stat icon={<Bell />} label="Live updates" value={summary.isLoading ? '—' : String(summary.data?.announcementCount ?? 0)} test="stat-announcements" /></div><div className="mt-14 grid gap-12 lg:grid-cols-[1.1fr_.9fr]"><section><div className="mb-6 flex items-end justify-between"><div><Eyebrow>From the organizers</Eyebrow><h2 className="display text-3xl font-bold tracking-[-.05em]">Fresh from the room.</h2></div><div className="flex rounded-full border border-foreground/10 p-1"><button type="button" onClick={() => setTab('all')} data-testid="button-news-all" className={`rounded-full px-3 py-1.5 text-xs font-bold ${tab === 'all' ? 'bg-foreground text-background' : ''}`}>All</button><button type="button" onClick={() => setTab('pinned')} data-testid="button-news-pinned" className={`rounded-full px-3 py-1.5 text-xs font-bold ${tab === 'pinned' ? 'bg-foreground text-background' : ''}`}>Pinned</button></div></div>{announcements.isLoading ? <LoadingCards /> : announcements.isError ? <ErrorState retry={() => announcements.refetch()} /> : news.length === 0 ? <div className="rounded-2xl border border-dashed p-10 text-center text-sm text-muted-foreground">No announcements yet. The first update will land here.</div> : <div className="space-y-3">{news.map((item) => <AnnouncementCard key={item.id} item={item} />)}</div>}</section><aside><div className="rounded-[1.5rem] bg-foreground p-6 text-background"><div className="flex items-center justify-between"><Eyebrow color="text-primary">Next event</Eyebrow><Clock3 className="size-5 text-primary" /></div>{summary.isLoading ? <div className="mt-6 h-28 animate-pulse rounded-xl bg-background/10" /> : summary.data?.nextEvent ? <><h2 className="mt-6 text-2xl font-bold leading-tight">{summary.data.nextEvent.title}</h2><p className="mt-3 text-sm leading-6 text-background/60">{summary.data.nextEvent.description}</p><div className="mono mt-7 flex items-center gap-2 text-xs text-primary"><CalendarDays className="size-4" />{summary.data.nextEvent.startTime} — {summary.data.nextEvent.endTime}</div><div className="mono mt-3 flex items-center gap-2 text-[10px] uppercase tracking-[.14em] text-background/40"><MapPin className="size-3" />{summary.data.nextEvent.location}</div></> : <p className="mt-4 text-sm text-background/60">The next event will appear here soon.</p>}</div><div className="mt-5 rounded-2xl border border-foreground/10 bg-muted p-5"><div className="flex items-center gap-2 text-sm font-bold"><span className="size-2 rounded-full bg-primary blink" />Live board</div><p className="mt-2 text-sm leading-6 text-muted-foreground">Keep this page open during the build day for changes and announcements.</p><Link href="/schedule" data-testid="link-dashboard-schedule" className="mt-4 flex items-center gap-2 text-sm font-bold underline underline-offset-4">View schedule <ArrowRight className="size-4" /></Link></div></aside></div></div></main></Shell>;
}
